...
describe('Dollar', function () {
  ...
  describe('mint', function () {
    describe('not from owner', function () {
      it('reverts', async function () {
        await expectRevert(this.dollar.mint(100, {from: userAddress}), "Ownable: caller is not the owner");
      });
    });

    describe('from reserve', function () {
      beforeEach(async function () {
        await this.dollar.mint(100, {from: ownerAddress});
      });

      it('mints new Dollar tokens', async function () {
        expect(await this.dollar.balanceOf(ownerAddress)).to.be.bignumber.equal(new BN(100));
      });
    });
  });
  ...
  describe('burn', function () {
    ...
    describe('not from owner', function () {
      it('reverts', async function () {
        await expectRevert(this.dollar.burn(100, {from: userAddress}), "Ownable: caller is not the owner");
      });
    });

    describe('from reserve', function () {
      beforeEach(async function () {
        await this.dollar.burn(100, {from: ownerAddress});
      });

      it('mints new Dollar tokens', async function () {
        expect(await this.dollar.balanceOf(ownerAddress)).to.be.bignumber.equal(new BN(0));
      });
    });
  });
  ...
  describe('delegate', function () {
    ...
    describe('valid expiration', function () {
      ...
      it('increments nonce', async function () {
        expect(await this.dollar.nonces(userAddress)).to.be.bignumber.equal(new BN(1));
      });
    });
