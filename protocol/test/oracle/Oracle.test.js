...
describe('Oracle', function () {
  ...
  beforeEach(async function () {
    ...
    this.oracle = await MockOracle.new(this.amm.address, this.usdc.address, this.dollar.address, {from: ownerAddress});
  ...
  describe('setup', function () {
    describe('not owner', function () {
      it('reverts', async function () {
        await expectRevert(this.oracle.setup(this.dollar.address, {from: userAddress}), "Ownable: caller is not the owner");
      });
    });
  });
  ...
  describe('step', function () {
    describe('not owner', function () {
      it('reverts', async function () {
        await expectRevert(this.oracle.capture(this.dollar.address, {from: userAddress}), "Ownable: caller is not the owner");
      });
    });

    describe('not setup', function () {
      beforeEach(async function () {
        this.oracle = await MockOracle.new(this.amm.address, this.usdc.address, ZERO_ADDRESS, {from: ownerAddress});
        await this.oracle.capture(this.dollar.address, {from: ownerAddress});
      });

      it('is not setup', async function () {
        expect(await this.oracle.setupFor(this.dollar.address)).to.be.equal(false);
        expect(await this.oracle.initializedFor(this.dollar.address)).to.be.equal(false);
        expect(await this.oracle.cumulativeFor(this.dollar.address)).to.be.bignumber.equal(new BN(0));
        expect(await this.oracle.timestampFor(this.dollar.address)).to.be.bignumber.equal(new BN(0));
      });
    });
    ...
    describe('after advance without trade', function () {
      ...
      it('is uninitialized', async function () {
        ...
        expect(await this.oracle.latestElapsed()).to.be.bignumber.equal(new BN(0));
        expect(await this.oracle.setupFor(this.dollar.address)).to.be.equal(true);
        expect(await this.oracle.initializedFor(this.dollar.address)).to.be.equal(false);
        expect(await this.oracle.cumulativeFor(this.dollar.address)).to.be.bignumber.equal(new BN(0));
        expect(await this.oracle.timestampFor(this.dollar.address)).to.be.bignumber.equal(new BN(0));
      });
      ...
      describe('price of 1', function () {
        describe('same block', function () {
          ...
          it('is initialized', async function () {
            ...
            expect(await this.oracle.latestElapsed()).to.be.bignumber.equal(new BN(0));
            expect(await this.oracle.setupFor(this.dollar.address)).to.be.equal(true);
            expect(await this.oracle.initializedFor(this.dollar.address)).to.be.equal(true);
            expect(await this.oracle.cumulativeFor(this.dollar.address)).to.be.bignumber.equal(new BN(0));
            expect(await this.oracle.timestampFor(this.dollar.address)).to.be.bignumber.equal(this.timestamp);
          });
        });
      ...
      describe('price of 1', function () {
        describe('same block', function () {
          ...
          it('is initialized', async function () {
            ...
            expect(await this.oracle.setupFor(this.dollar.address)).to.be.equal(true);
            expect(await this.oracle.initializedFor(this.dollar.address)).to.be.equal(true);
            expect(await this.oracle.cumulativeFor(this.dollar.address)).to.be.bignumber.equal(uint112s(this.timediff));
            expect(await this.oracle.timestampFor(this.dollar.address)).to.be.bignumber.equal(this.timestamp);
          });
        });
        ...
        describe('long before', function () {
          ...
          it('is initialized', async function () {
            expect(await priceForToBN(this.oracle)).to.be.bignumber.closeTo(cents(100), EPSILON);
            expect(await this.oracle.latestElapsed()).to.be.bignumber.equal(new BN(90000));
            expect(await this.oracle.setupFor(this.dollar.address)).to.be.equal(true);
            expect(await this.oracle.initializedFor(this.dollar.address)).to.be.equal(true);
            expect(await this.oracle.cumulativeFor(this.dollar.address)).to.be.bignumber.equal(uint112s(this.timediff));
            expect(await this.oracle.timestampFor(this.dollar.address)).to.be.bignumber.equal(this.timestamp);
      ...
      describe('price greater than 1', function () {
        describe('same block', function () {
          ...
          it('is initialized', async function () {
            ...
            expect(await this.oracle.setupFor(this.dollar.address)).to.be.equal(true);
            expect(await this.oracle.initializedFor(this.dollar.address)).to.be.equal(true);
            expect(await this.oracle.cumulativeFor(this.dollar.address)).to.be.bignumber.equal(uint112s(this.timediff, 1100000, 1000000));
            expect(await this.oracle.timestampFor(this.dollar.address)).to.be.bignumber.equal(this.timestamp);
          });
        });
      ...
      describe('price greater than 1', function () {
        ...
        describe('different prices', function () {
          ...
          it('is initialized', async function () {
            ...
            expect(await this.oracle.setupFor(this.dollar.address)).to.be.equal(true);
            expect(await this.oracle.initializedFor(this.dollar.address)).to.be.equal(true);
            expect(await this.oracle.cumulativeFor(this.dollar.address)).to.be.bignumber.equal(uint112s(this.timediff, 1150000, 1000000));
            expect(await this.oracle.timestampFor(this.dollar.address)).to.be.bignumber.equal(this.timestamp);
          });
        });
      });
      ...
      describe('price greater than 1', function () {
        ...
        describe('different prices', function () {
          ...
          it('is initialized', async function () {
            ...
            expect(await this.oracle.setupFor(this.dollar.address)).to.be.equal(true);
            expect(await this.oracle.initializedFor(this.dollar.address)).to.be.equal(true);
            expect(await this.oracle.cumulativeFor(this.dollar.address)).to.be.bignumber.equal(begin.add(end));
            expect(await this.oracle.timestampFor(this.dollar.address)).to.be.bignumber.equal(this.timestamp);
          });
        });
      });
    ...
    describe('no time between advances', function () {
      describe('different prices', function () {
        beforeEach(async function () {
          await simulateTrade(this.amm, 1000000, 950000);
          await this.oracle.capture(this.dollar.address, {from: ownerAddress});
    ...
    describe('current reserve too low', function () {
      describe('long before', function () {
        ...
        it('is initialized', async function () {
          ...
          expect(await this.oracle.setupFor(this.dollar.address)).to.be.equal(true);
          expect(await this.oracle.initializedFor(this.dollar.address)).to.be.equal(true);
          expect(await this.oracle.cumulativeFor(this.dollar.address)).to.be.bignumber.equal(uint112s(this.timediff, 3000, 2500));
          expect(await this.oracle.timestampFor(this.dollar.address)).to.be.bignumber.equal(this.timestamp);
        });
      });
    });
