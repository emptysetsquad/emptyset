// Fork of https://github.com/OpenZeppelin/openzeppelin-contracts/blob/v2.5.0/test/drafts/TokenVesting.test.js
// Changes to arguments of modified release calls
...
const TokenVestingImpl = contract.fromArtifact('Vester');
...
describe('TokenVesting', function () {
  ...
  it('reverts with a duration shorter than the cliff', async function () {
    ...
    await expectRevert(
      TokenVestingImpl.new(beneficiary, this.start, cliffDuration, duration, true, { from: owner }),
  ...
  it('reverts with a null beneficiary', async function () {
    await expectRevert(
      TokenVestingImpl.new(ZERO_ADDRESS, this.start, this.cliffDuration, this.duration, true, { from: owner }),
  ...
  it('reverts with a null duration', async function () {
    ...
    await expectRevert(
      TokenVestingImpl.new(beneficiary, this.start, 0, 0, true, { from: owner }), 'TokenVesting: duration is 0'
    );
  });
  ...
  it('reverts if the end time is in the past', async function () {
    ...
    await expectRevert(
      TokenVestingImpl.new(beneficiary, this.start, this.cliffDuration, this.duration, true, { from: owner }),
  ...
  context('once deployed', function () {
    beforeEach(async function () {
      this.vesting = await TokenVestingImpl.new(
        beneficiary, this.start, this.cliffDuration, this.duration, true, { from: owner });

      this.token = await MockToken.new("MOCK", "MOCK", 18, { from: owner });
    ...
    it('cannot be released before cliff', async function () {
      await expectRevert(this.vesting.release(this.token.address, MAX_UINT256, { from: beneficiary }),
    ...
    it('can be released after cliff', async function () {
      ...
      const { logs } = await this.vesting.release(this.token.address, MAX_UINT256, { from: beneficiary });
    ...
    it('should release proper amount after cliff', async function () {
      ...
      await this.vesting.release(this.token.address, MAX_UINT256, { from: beneficiary });
    ...
    it('should fail to be revoked by owner if revocable not set', async function () {
      const vesting = await TokenVestingImpl.new(
